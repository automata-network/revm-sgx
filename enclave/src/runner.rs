...
use serde_json::json;
use std::{
    collections::HashMap, convert::Infallible, path::{Path, PathBuf}, sync::{atomic::{AtomicBool, Ordering}, Arc, Mutex}, time::{Duration, Instant}
};
use thiserror::Error;
...
pub fn execute_test_suite(
    ...
    for (name, unit) in suite.0 {
        ...
        for (address, info) in unit.pre {
            ...
            let h = HashMap::from_iter(info.storage.into_iter());
...
pub fn run(
    ...
    for i in 0..num_threads {
        ...
        let f = move || loop {
            ...
            println!("exec test suite: {}", test_path.display());
            if let Err(err) = execute_test_suite(&test_path, &elapsed, print_outcome) {
    ...
    //     println!("exec test suite: {}", test_path.display());
    //     if let Err(err) = execute_test_suite(&test_path, &elapsed2, print_outcome) {
    //         endjob.store(true, Ordering::SeqCst);
    //         return Err(err);
    //     }
    // };
