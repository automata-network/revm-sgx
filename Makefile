...
enclave: export CARGO_FEATURE_STD=1
ifeq ($(BUILD_STD), cargo)
	Rust_Build_Std := $(Rust_Build_Flags) -Z build-std=core,alloc
	Rust_Std_Features := --features env,net,thread,untrusted_time,untrusted_fs,unsupported_process,capi
...
$(App_Name): $(App_Enclave_u_Object) app
	...
	@cp $(App_Rust_Path)/server $(CUSTOM_BIN_PATH)
	@echo "LINK => $@"
